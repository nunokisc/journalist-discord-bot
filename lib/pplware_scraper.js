...
module.exports = {
  getTopArticle: function (callback) {
    ...
    axios.get(url, { headers: { 'User-Agent': userAgent }, timeout: 10000 }).then(response => {
      ...
      if (!articleScraped.id || !articleScraped.url) {
        callback(new Error('Pplware page markup did not match expected structure'));
        return;
      }
